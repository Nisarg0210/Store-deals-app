...
import { useAuthState, signIn, signOut } from '@/lib/auth';
  ...
  if (!isStaffEmail(user.email)) {
    return (
      <>
        <div className="bg-mesh" />
        <div className="login-page">
          <div className="login-card card animate-fadeInUp" style={{ maxWidth: 420, margin: 'auto' }}>
            <div className="login-card__top">
              <div className="login-logo">🔒</div>
              <h2 className="login-title">Staff access only</h2>
              <p className="login-sub">
                Signed in as {user.email}. This area is for store staff only.
              </p>
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
              <a href="/rewards" className="btn btn-primary btn-lg" style={{ justifyContent: 'center' }}>
                Go to my rewards
              </a>
              <button
                type="button"
